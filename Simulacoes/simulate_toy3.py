...
simulate_toy3.py
----------------
Simulação sintética da "Solução 3" (toy1_3.ini) *sem* ler o .ini:
- 3 clusters CoMP:
    • [4,5,6,7]  ancorado no 4  e na CU2
    • [8,9,10]   ancorado no 8  e na CU1
    • [11,12]    ancorado no 11 e na CU3
- Nenhum gNB em D-RAN
- Geração automática de TABELAS (.csv) e GRÁFICOS (.png)
- Saída organizada em: /home/felipe/Documentos/tcc/omnet/Topologias/Resultados/Topologia3/<timestamp>/

Observação: modelo energético idêntico ao usado no toy2 para manter comparabilidade.
Ajuste coeficientes em `EnergyCoeffs` e `CompTuning` conforme necessário.

Como executar (exemplo):
    python3 simulate_toy3.py \
      --outdir /home/felipe/Documentos/tcc/omnet/Topologias/Resultados/Topologia3 \
      --tx 20,23,26,29,32
"""
...
# Parâmetros gerais
# =============================================================
# Pasta base de resultados desta topologia (pode ser alterada via --outdir)
RESULT_BASE = Path("/home/felipe/Documentos/tcc/omnet/Topologias/Resultados/Topologia3")
SIM_NAME = "toy3"
DEFAULT_TX_POWERS_DBM = [20, 23, 26, 29, 32]

# Topologia "Solução 3" (CoMP clusters)
CLUSTERS = [
    {"members": [4, 5, 6, 7],  "anchor_gnb": 4,  "anchor_cu": "CU2"},
    {"members": [8, 9, 10],    "anchor_gnb": 8,  "anchor_cu": "CU1"},
    {"members": [11, 12],      "anchor_gnb": 11, "anchor_cu": "CU3"},
]
DRAN_GNBS: List[int] = []  # nenhum gNB em D-RAN neste cenário

# Parâmetros de carga (ajustáveis por você, se quiser via código)
UES_PER_GNB = 10
BASE_THROUGHPUT_PER_UE = 5.0  # Mbps

# =============================================================
# Modelo de energia (mesma base do toy2 para comparabilidade)
# =============================================================
@dataclass
class EnergyCoeffs:
    ...
    k_tx: float = 0.8            # W por dBm (aprox. do PA/linha RF)
...
def energy_dran(ues: int, tx_dbm: float) -> float:
    """Definido para manter compatibilidade com outros cenários (não usado aqui)."""
...
def plot_energy_vs_power(df_all: pd.DataFrame, outdir: Path):
    ...
    plt.title("Consumo de energia por configuração (toy3)")
    plt.grid(True, linestyle=":"); plt.legend(); plt.tight_layout()
    plt.savefig(outdir / "energia_vs_potencia_toy3.png", dpi=300)
...
def plot_efficiency_vs_power(df_all: pd.DataFrame, outdir: Path):
    ...
    plt.title("Eficiência energética por configuração (toy3)")
    plt.grid(True, linestyle=":"); plt.legend(); plt.tight_layout()
    plt.savefig(outdir / "eficiencia_vs_potencia_toy3.png", dpi=300)
...
def plot_stack_energy(df_all: pd.DataFrame, outdir: Path):
    ...
    # Ordem desejada nas colunas (aqui: CLUSTER + CU_CLUSTER)
    ...
    plt.xlabel("Potência (dBm)"); plt.ylabel("Energia (W)")
    plt.title("Decomposição da energia (toy3)")
    plt.grid(True, axis="y", linestyle=":"); plt.tight_layout()
    plt.savefig(outdir / "stack_energy_breakdown_toy3.png", dpi=300)

# =============================================================
# Main
# =============================================================
def main():
    parser = argparse.ArgumentParser(description="Simulação sintética do cenário toy3 (Solução 3).")
    parser.add_argument("--outdir", type=str, default=None,
                        help="Pasta base de saída (default Topologia3)")
    ...
    # Tabelas
    df_all.to_csv(outdir / "resultados_toy3_detalhado.csv", index=False)
    df_all.groupby(["TX_dBm"]).agg(
        Energia_W=("Energia_W", "sum"),
        Throughput_Mbps=("Throughput_Mbps", "sum")
    ).to_csv(outdir / "resultados_toy3_resumo_por_potencia.csv")
    ...
    with open(outdir / "metadata_toy3.json", "w") as f:
    ...
    print(" - resultados_toy3_detalhado.csv")
    print(" - resultados_toy3_resumo_por_potencia.csv")
    print(" - energia_vs_potencia_toy3.png")
    print(" - eficiencia_vs_potencia_toy3.png")
    print(" - stack_energy_breakdown_toy3.png")
    print(" - metadata_toy3.json")
