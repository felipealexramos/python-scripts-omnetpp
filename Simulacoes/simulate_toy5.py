...
simulate_toy5.py
----------------
Simulação sintética da "Solução 5" (toy1_5.ini) *sem ler o .ini*:
- 1 cluster CoMP:
    • [6,7,8,9,10] ancorado no 6 e na CU1
- CU2 e CU3 desligadas (não contabilizam energia)
- gNBs em D-RAN: 4, 5, 11, 12
- Geração automática de TABELAS (.csv) e GRÁFICOS (.png)
- Saída: /home/felipe/Documentos/tcc/omnet/Topologias/Resultados/Topologia5/<timestamp>/

Como executar (exemplo):
    python3 simulate_toy5.py \
      --outdir /home/felipe/Documentos/tcc/omnet/Topologias/Resultados/Topologia5 \
      --tx 20,23,26,29,32
"""
...
# Parâmetros gerais
# =============================================================
RESULT_BASE = Path("/home/felipe/Documentos/tcc/omnet/Topologias/Resultados/Topologia5")
SIM_NAME = "toy5"
DEFAULT_TX_POWERS_DBM = [20, 23, 26, 29, 32]

# Topologia "Solução 5"
CLUSTERS = [
    {"members": [6, 7, 8, 9, 10], "anchor_gnb": 6, "anchor_cu": "CU1"},
]
DRAN_GNBS: List[int] = [4, 5, 11, 12]
...
# =============================================================
# Modelo de energia (mesma base do toy2/3/4 para comparabilidade)
# =============================================================
@dataclass
class EnergyCoeffs:
...
def energy_cluster(members: List[int], ues_per_gnb: int, tx_dbm: float):
    ...
    for g in members:
        ...
        total += e
    total += EC.P_idle_CU  # CU fixa do cluster (apenas CU1 ativa neste cenário)
...
def plot_energy_vs_power(df_all: pd.DataFrame, outdir: Path):
    ...
    plt.title("Consumo de energia por configuração (toy5)")
    plt.grid(True, linestyle=":"); plt.legend(); plt.tight_layout()
    plt.savefig(outdir / "energia_vs_potencia_toy5.png", dpi=300)
...
def plot_efficiency_vs_power(df_all: pd.DataFrame, outdir: Path):
    ...
    plt.title("Eficiência energética por configuração (toy5)")
    plt.grid(True, linestyle=":"); plt.legend(); plt.tight_layout()
    plt.savefig(outdir / "eficiencia_vs_potencia_toy5.png", dpi=300)
...
def plot_stack_energy(df_all: pd.DataFrame, outdir: Path):
    ...
    plt.xlabel("Potência (dBm)"); plt.ylabel("Energia (W)")
    plt.title("Decomposição da energia (toy5)")
    plt.grid(True, axis="y", linestyle=":"); plt.tight_layout()
    plt.savefig(outdir / "stack_energy_breakdown_toy5.png", dpi=300)

# =============================================================
# Main
# =============================================================
def main():
    parser = argparse.ArgumentParser(description="Simulação sintética do cenário toy5 (Solução 5).")
    parser.add_argument("--outdir", type=str, default=None,
                        help="Pasta base de saída (default Topologia5)")
    ...
    # Tabelas
    df_all.to_csv(outdir / "resultados_toy5_detalhado.csv", index=False)
    df_all.groupby(["TX_dBm"]).agg(
        Energia_W=("Energia_W", "sum"),
        Throughput_Mbps=("Throughput_Mbps", "sum")
    ).to_csv(outdir / "resultados_toy5_resumo_por_potencia.csv")
    ...
    with open(outdir / "metadata_toy5.json", "w") as f:
    ...
    print(" - resultados_toy5_detalhado.csv")
    print(" - resultados_toy5_resumo_por_potencia.csv")
    print(" - energia_vs_potencia_toy5.png")
    print(" - eficiencia_vs_potencia_toy5.png")
    print(" - stack_energy_breakdown_toy5.png")
    print(" - metadata_toy5.json")
