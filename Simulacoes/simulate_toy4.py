...
simulate_toy4.py
----------------
Simulação sintética da "Solução 4" (toy1_4.ini) *sem ler o .ini*:
- 2 clusters CoMP:
    • [4,5,6,7]  ancorado no 4  e na CU2
    • [9,10,11,12] ancorado no 9 e na CU3
- CU1 desligada (não contabiliza energia)
- gNB em D-RAN: 8
- Geração automática de TABELAS (.csv) e GRÁFICOS (.png)
- Saída: /home/felipe/Documentos/tcc/omnet/Topologias/Resultados/Topologia4/<timestamp>/

Como executar (exemplo):
    python3 simulate_toy4.py \
      --outdir /home/felipe/Documentos/tcc/omnet/Topologias/Resultados/Topologia4 \
      --tx 20,23,26,29,32
"""
...
# Parâmetros gerais
# =============================================================
RESULT_BASE = Path("/home/felipe/Documentos/tcc/omnet/Topologias/Resultados/Topologia4")
SIM_NAME = "toy4"
...
CLUSTERS = [
    ...
    {"members": [9, 10, 11, 12],   "anchor_gnb": 9, "anchor_cu": "CU3"},
]
DRAN_GNBS: List[int] = [8]  # único D-RAN

# Parâmetros de carga (mantidos fixos por enquanto)
UES_PER_GNB = 10
BASE_THROUGHPUT_PER_UE = 5.0  # Mbps

# =============================================================
# Modelo de energia (mesma base do toy2/3 para comparabilidade)
# =============================================================
@dataclass
class EnergyCoeffs:
...
def simulate_one_power(tx_dbm: float, max_tx: float) -> pd.DataFrame:
    ...
    # Clusters CoMP (CU1 está desligada e não aparece aqui)
...
def plot_energy_vs_power(df_all: pd.DataFrame, outdir: Path):
    ...
    plt.title("Consumo de energia por configuração (toy4)")
    plt.grid(True, linestyle=":"); plt.legend(); plt.tight_layout()
    plt.savefig(outdir / "energia_vs_potencia_toy4.png", dpi=300)
...
def plot_efficiency_vs_power(df_all: pd.DataFrame, outdir: Path):
    ...
    plt.title("Eficiência energética por configuração (toy4)")
    plt.grid(True, linestyle=":"); plt.legend(); plt.tight_layout()
    plt.savefig(outdir / "eficiencia_vs_potencia_toy4.png", dpi=300)
...
def plot_stack_energy(df_all: pd.DataFrame, outdir: Path):
    ...
    plt.xlabel("Potência (dBm)"); plt.ylabel("Energia (W)")
    plt.title("Decomposição da energia (toy4)")
    plt.grid(True, axis="y", linestyle=":"); plt.tight_layout()
    plt.savefig(outdir / "stack_energy_breakdown_toy4.png", dpi=300)

# =============================================================
# Main
# =============================================================
def main():
    parser = argparse.ArgumentParser(description="Simulação sintética do cenário toy4 (Solução 4).")
    parser.add_argument("--outdir", type=str, default=None,
                        help="Pasta base de saída (default Topologia4)")
    ...
    # Tabelas
    df_all.to_csv(outdir / "resultados_toy4_detalhado.csv", index=False)
    df_all.groupby(["TX_dBm"]).agg(
        Energia_W=("Energia_W", "sum"),
        Throughput_Mbps=("Throughput_Mbps", "sum")
    ).to_csv(outdir / "resultados_toy4_resumo_por_potencia.csv")
    ...
    with open(outdir / "metadata_toy4.json", "w") as f:
    ...
    print(" - resultados_toy4_detalhado.csv")
    print(" - resultados_toy4_resumo_por_potencia.csv")
    print(" - energia_vs_potencia_toy4.png")
    print(" - eficiencia_vs_potencia_toy4.png")
    print(" - stack_energy_breakdown_toy4.png")
    print(" - metadata_toy4.json")
